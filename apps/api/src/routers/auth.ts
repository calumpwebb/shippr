...
import { users, passwordResetTokens, eq } from '@shippr/db'
  ...
  createUser: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        const token = await ctx.jwt.generateToken({ userId: user.id, email: user.email })
  ...
  resetPassword: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (new Date() > expiresAt) {
        await ctx.db.delete(passwordResetTokens).where(eq(passwordResetTokens.id, token.id))
        throw new TRPCError({
          code: 'BAD_REQUEST',
